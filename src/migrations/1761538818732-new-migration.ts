...
export class NewMigration1761538818732 implements MigrationInterface {
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(
      `ALTER TABLE \`game\` ADD \`type\` varchar(50) NULL`,
    );
    await queryRunner.query(
      `ALTER TABLE \`offers\` ADD \`shoppingPlatform\` varchar(100) NOT NULL`,
    );
    await queryRunner.query(
      `ALTER TABLE \`offers\` ADD \`promotionsPrice\` decimal(12,2) NULL`,
    );
    await queryRunner.query(
  ...
  public async down(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(
      `ALTER TABLE \`offers\` DROP COLUMN \`promotionsPrice\``,
    );
    await queryRunner.query(
      `ALTER TABLE \`offers\` DROP COLUMN \`shoppingPlatform\``,
    );
